...
void shader_uniform_material(Shader self, Material material) { 
    shader_uniform_vec3(self, "material.diffuse", material.diffuse);
    shader_uniform_vec3(self, "material.specular", material.specular);
    shader_uniform_float(self, "material.shine", material.shine);
}

void shader_uniform_point_light(Shader self, PointLight light, u32 n) {
        char buffer[64];     
        snprintf(buffer, 64, "point_lights[%u].ambient", n);
        shader_uniform_vec3(self, buffer, light.ambient);
        snprintf(buffer, 64, "point_lights[%u].diffuse", n);
        shader_uniform_vec3(self, buffer, light.diffuse);
        snprintf(buffer, 64, "point_lights[%u].specular", n);
        shader_uniform_vec3(self, buffer, light.specular);
        snprintf(buffer, 64, "point_lights[%u].position", n);
        shader_uniform_vec3(self, buffer, light.position);
        snprintf(buffer, 64, "point_lights[%u].constant", n);
        shader_uniform_float(self, buffer, light.constant);
        snprintf(buffer, 64, "point_lights[%u].linear", n);
        shader_uniform_float(self, buffer, light.linear);
        snprintf(buffer, 64, "point_lights[%u].quadratic", n);
        shader_uniform_float(self, buffer, light.quadratic);
}

...
static void _mouse_callback(GLFWwindow *handle, int button, int action,
                            int mods) {
    if (button < 0) {
        return;
    }
...
void window_create(FWindow init, FWindow destroy, FWindow tick, FWindow update,
                   FWindow render) {
    window.init = init;
...
//    glfwWindowHint(GLFW_SAMPLES, 4);
    ...
    window.handle =
        glfwCreateWindow(window.size.x, window.size.y, "Project", NULL, NULL);

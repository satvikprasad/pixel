...
void perspective_camera_update(struct PerspectiveCamera *self) {
    ...
    self->pitch = clamp(self->pitch, radians(-80.f), radians(80.f));
    self->yaw = (self->yaw < 0 ? TAU : 0.0f) + fmodf(self->yaw, TAU);

    self->direction =
        (vec3s){{cosf(self->pitch) * sinf(self->yaw), sinf(self->pitch),
    ...
    self->view_proj.view =
        glms_lookat(self->position,
                    glms_vec3_add(self->position, self->direction), self->up);
    self->view_proj.proj =
        glms_perspective(self->fov, self->aspect, self->znear, self->zfar);
}
...
void ortho_camera_update(struct OrthoCamera *self) {
    ...
    self->view_proj.proj = glms_ortho(self->min.x, self->max.x, self->min.y,
                                      self->max.y, -100.0f, 100.0f);
}

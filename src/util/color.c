...
vec3s rgb2xyz(vec3s c) {
    ...
    const mat3s mat = (mat3s){.col = {(vec3s){{0.4124, 0.3576, 0.1805}},
...
vec3s xyz2lab(vec3s c) {
    ...
    return (vec3s){
        {(116.0 * v.y) - 16.0, 500.0 * (v.x - v.y), 200.0 * (v.y - v.z)}};
}

vec3s rgb2lab(vec3s c) {
    vec3s lab = xyz2lab(rgb2xyz(c));
    return (vec3s){{lab.x / 100.0, 0.5 + 0.5 * (lab.y / 127.0),
                    0.5 + 0.5 * (lab.z / 127.0)}};
}
...
vec3s lab2xyz(vec3s c) {
    ...
    f32 fx = c.y / 500.0 + fy;
    f32 fz = fy - c.z / 200.0;
    return (vec3s){{95.047 * ((fx > 0.206897) ? fx * fx * fx
                                              : (fx - 16.0 / 116.0) / 7.787),
                    100.000 * ((fy > 0.206897) ? fy * fy * fy
                                               : (fy - 16.0 / 116.0) / 7.787),
                    108.883 * ((fz > 0.206897) ? fz * fz * fz
                                               : (fz - 16.0 / 116.0) / 7.787)}};
}

vec3s xyz2rgb(vec3s c) {
    const mat3s mat = (mat3s){.col = {(vec3s){{3.2406, -1.5372, -0.4986}},
                                      (vec3s){{-0.9689, 1.8758, 0.0415}},
                                      (vec3s){{0.0557, -0.2040, 1.0570}}}};
    vec3s v = glms_mat3_mulv(mat, glms_vec3_scale(c, 1.0f / 100.0f));
    vec3s r;
    r.x = (v.x > 0.0031308) ? ((1.055 * pow(v.x, (1.0 / 2.4))) - 0.055)
                            : 12.92 * v.x;
    r.y = (v.y > 0.0031308) ? ((1.055 * pow(v.y, (1.0 / 2.4))) - 0.055)
                            : 12.92 * v.y;
    r.z = (v.z > 0.0031308) ? ((1.055 * pow(v.z, (1.0 / 2.4))) - 0.055)
                            : 12.92 * v.z;
    return r;
}

vec3s lab2rgb(vec3s c) {
    return xyz2rgb(lab2xyz((vec3s){
        {100.0 * c.x, 2.0 * 127.0 * (c.y - 0.5), 2.0 * 127.0 * (c.z - 0.5)}}));
}
...
vec4s rgba_brighten(vec4s rgba, f32 d) {
    return glms_vec4(rgb_brighten((vec3s){{rgba.x, rgba.y, rgba.z}}, d),
                     rgba.w);
}
...
vec4s rgba_lerp(vec4s rgba_a, vec4s rgba_b, f32 t) {
    ...
    return glms_vec4(lab2rgb(((vec3s){{(lerpf(lab_a.x, lab_b.x, t)),
